...
/**
 * Preloads the {@link Representations} associations required by the view layer.
 * 
 * Do not use other GraphLoaders in this GraphLoader. 
 */
@Named
@Singleton
public class RepresentationsGraphLoader extends EntityGraphLoader<Representations, Integer> {
    // required by cglib to create a proxy around the object as we are using the @Transactional annotation
    protected RepresentationsGraphLoader() {
        super();
    }

    @Inject
    public RepresentationsGraphLoader(RepresentationsRepository representationsRepository) {
        super(representationsRepository);
    }

    @Override
    public void loadGraph(Representations representations) {
        loadSingular(representations.getShow());
        loadSingular(representations.getLocation());
    }
}