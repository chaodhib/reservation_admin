...
/**
 * Exports to excel document {@link Types} search criteria and result. 
 */
@Named
@ConversationContextScoped
public class TypesExcelExporter extends GenericExcelExporter<Types> {
    @Inject
    protected TypesSearchForm sf;

    public TypesExcelExporter() {
        super("types_type");
    }

    @Override
    protected void fillResultItem(int row, Types item) {
        int col = 0;
        setValue(row, col++, item.getType());
    }
    ...
    public void fillSearchCriteria(int row) {
        ...
        setSelector(row++, 0, "types_type", sf.getTypeSelector());
    }
}